...
if(window.performance.navigation.type>0){
	...
	window.localStorage.setItem("back",false);	
 	...
 	/* Code for getting free session from accesscoe */
...
if(window.localStorage.getItem("Accessbundle")){
	
var accbnd = window.localStorage.getItem("Accessbundle");
var ACCBnd = accbnd.split(',');
//alert(ACCBnd);
}
						...
						$.map(value.Bundle, function(value2, index2) {
							...
							$.map(user.streak, function(value3, index3) {
								...
								cp.push(comper);
									...
									if((user.membership_type != "Free" ||  $.inArray(value2.bundle_id,ACCBnd) != -1) || $.inArray(key,ACCCAT) != -1){
									...
									if((user.membership_type != "Free" || $.inArray(value2.bundle_id,final) != -1 ) ||  ($.inArray(value2.bundle_id,ACCBnd) != -1 || $.inArray(key,ACCCAT) != -1)){
...
if(window.check == true || ($.inArray(value2.bundle_id,ACCBnd) != -1 || $.inArray(key,ACCCAT) != -1)){
...
var final = JSON.parse(window.localStorage.getItem("cove_data2"));
		...
		if($.inArray(id,final) != -1 || user.membership_type != "Free"){
			...
			if($(this).data("total") > 1){
				if($.inArray(id,ACCBnd) != -1 || $.inArray(ct,ACCCAT) != -1){
			...
			$(".subscribe").click(function(){
				...
				$("#memberModal").modal("hide");
					$.post("test.php", {"price": price}, function(result){
			        console.log(result);
