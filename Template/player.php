...
//alert(conversation+"=="+window.localStorage.getItem('content'));
        console.log(conversation+'=='+ window.localStorage.getItem('content')+'='+storedNames[i][j].session_name);
		//alert(storedNames[i][j].meditation_audio_time.length);
				if(storedNames[i][j].meditation_audio_time.length > 2){
						$(".timeslot").show();
						var ts = '';
						var l = 0;
						for(time in storedNames[i][j].meditation_audio_time){
							var t = storedNames[i][j].meditation_audio_time[time];
...
//alert(vid.currentTime);
  ...
  $('.cp-play').css("display","inherit");
		  ...
		  if(window.localStorage.getItem('cat') != '10 Day Intro Program' && window.localStorage.getItem('cat')){
				  var s = JSON.parse(window.localStorage.getItem("session"));
				  for(t in s.meditation_audio_time){
					if(s.meditation_audio_time[t] == $(this).attr('id') && $(this).index() == t){
			...
			//alert(vid.currentTime);
				  ...
				  //alert($(this).attr('id'));
		}else if(( window.localStorage.getItem('cat') == '10 Day Intro Program' && window.localStorage.getItem("content") > 3) || (!window.localStorage.getItem('cat') && window.localStorage.getItem("content") > 3)){
							var s = JSON.parse(window.localStorage.getItem("session"));
							var conversation = 1;
					        for (i in s)
							{
							   for(j in s[i]){
								    if(conversation == window.localStorage.getItem('content'))
									{
										   ...
										   for(t in s[i][j].meditation_audio_time)
											{
												if(s[i][j].meditation_audio_time[t] == $(this).attr('id') && $(this).index() == t){

												  vid.src = s[i][j].meditation_audio[t];
												   if (vid.paused == false) {
													vid.src = s[i][j].meditation_audio[t];
												  }
   ...
   $(".journaladd").click(function(){
    ...
    // }
            ...
            if(user.membership_type != 'Free'){
              ...
              //db.ref("Users/"+user.user_id+"/last_free_conversation_id").set(0).then(function(snap){
...
//         }
       ...
       if((!window.localStorage.getItem("cat") || window.localStorage.getItem("cat") == "10 Day Intro Program") && (user.last_free_conversation_id == 3 || user.last_free_conversation_id == 7 || user.last_free_conversation_id == 9) && user.membership_type == "Free"){
         ...
         //  $.redirect("playafter.php",{"nm":Sessiondata.session_name,"qimg":Sessiondata.session_quote_img,"qdesc":Sessiondata.session_quote_description},"POST",null,null,true);
           $.redirect("Summary",{"nm":Sessiondata.session_name,"qimg":Sessiondata.session_quote_img,"qdesc":Sessiondata.session_quote_description},"POST",null,null,true);
